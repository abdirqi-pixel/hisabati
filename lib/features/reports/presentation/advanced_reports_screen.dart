            ...
            children: [
              ...
              report.when(
                data: (data) {
                  ...
                  final byProject = (data['byProject'] as List).cast<Map<String, Object?>>();
                  final expenses = (data['expenses'] as List).cast<Map<String, Object?>>();
                    ...
                    children: [
                      Container(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('نتيجة التقرير', style: TextStyle(color: Colors.white70)),
                          ...
                          Expanded(
                            child: FilledButton.icon(
                              onPressed: () async {
                                ...
                                await Share.shareXFiles([XFile(file.path)], text: 'تقرير حساباتي المتقدم PDF');
                              },
                          ...
                          Expanded(
                            child: FilledButton.tonalIcon(
                              onPressed: () async {
                                ...
                                await Share.shareXFiles([XFile(file.path)], text: 'تقرير حساباتي المتقدم Excel');
                              },
                      ...
                      SimpleBarChart(
                        title: 'حسب المشروع',
                        items: byProject.map((e) => ChartItem(label: e['name'].toString(), value: (e['total'] as num?) ?? 0)).toList(),
                        valueLabelBuilder: (value) => MoneyFormatter.format(value, symbol),
                      ),
                      const SizedBox(height: 14),
                      SimpleBarChart(
                        title: 'حسب الشخص',
                        items: byPerson.map((e) => ChartItem(label: e['name'].toString(), value: (e['total'] as num?) ?? 0)).toList(),
                        valueLabelBuilder: (value) => MoneyFormatter.format(value, symbol),
                      ),
                      const SizedBox(height: 14),
                      SimpleBarChart(
                        title: 'حسب التصنيف',
                        items: byCategory.map((e) => ChartItem(label: e['name'].toString(), value: (e['total'] as num?) ?? 0)).toList(),
                        valueLabelBuilder: (value) => MoneyFormatter.format(value, symbol),
                      ),
                      const SizedBox(height: 18),
                      const Text('العمليات المطابقة', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 10),
                      if (expenses.isEmpty)
                        const Card(child: Padding(padding: EdgeInsets.all(18), child: Text('لا توجد عمليات مطابقة')))
                      else
                        ...expenses.take(100).map((e) {
                          return Card(
                            child: ListTile(
                              ...
                              subtitle: Text('${e['project_name'] ?? 'بدون مشروع'} • ${e['person_name'] ?? 'بدون شخص'} • ${e['category_name'] ?? 'بدون تصنيف'}\n${e['description'] ?? ''}'),
                              isThreeLine: true,
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('الفلاتر', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
            ...
            projects.when(
              data: (items) => DropdownButtonFormField<int>(
                ...
                items: [
                  const DropdownMenuItem<int>(value: null, child: Text('كل المشاريع')),
                  ...items.map((p) => DropdownMenuItem<int>(value: p['id'] as int, child: Text(p['name'].toString()))),
                ],
                onChanged: (value) => ref.read(reportFilterProvider.notifier).state =
                    ref.read(reportFilterProvider).copyWith(projectId: value, clearProject: value == null),
              ),
            ...
            persons.when(
              data: (items) => DropdownButtonFormField<int>(
                ...
                onChanged: (value) => ref.read(reportFilterProvider.notifier).state =
                    ref.read(reportFilterProvider).copyWith(personId: value, clearPerson: value == null),
              ),
              loading: () => const LinearProgressIndicator(),
              error: (e, _) => Text('$e'),
            ),
            ...
            categories.when(
              data: (items) => DropdownButtonFormField<int>(
                ...
                items: [
                  const DropdownMenuItem<int>(value: null, child: Text('كل التصنيفات')),
                  ...items.map((c) => DropdownMenuItem<int>(value: c['id'] as int, child: Text(c['name'].toString()))),
                ],
                onChanged: (value) => ref.read(reportFilterProvider.notifier).state =
                    ref.read(reportFilterProvider).copyWith(categoryId: value, clearCategory: value == null),
              ),
              loading: () => const LinearProgressIndicator(),
              error: (e, _) => Text('$e'),
            ),
              ...
              children: [
                Expanded(
                  child: TextField(
                    ...
                    onChanged: (value) => ref.read(reportFilterProvider.notifier).state =
                        ref.read(reportFilterProvider).copyWith(amountFrom: double.tryParse(value), clearAmounts: value.isEmpty && amountTo.text.isEmpty),
                  ),
                ),
                const SizedBox(width: 10),
                Expanded(
                  child: TextField(
                    ...
                    onChanged: (value) => ref.read(reportFilterProvider.notifier).state =
                        ref.read(reportFilterProvider).copyWith(amountTo: double.tryParse(value), clearAmounts: value.isEmpty && amountFrom.text.isEmpty),
                  ),
                ),
              ],
            ),
